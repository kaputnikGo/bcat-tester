...
#define SPECIAL_MODES_REG                 18
#define MIIMODE_BIT                       14
...
// get the special modes register as 16 bit binary
void smi_special_modes_reg_state(smi_interface_t &smi, char *modes) {
    for (int i = 0; i <= 15; i++) {
       modes[i] = (smi_reg(smi, SPECIAL_MODES_REG, 0, SMI_READ) >> i) & 1;
    }
}

// 0 = MII mode, 1 = RMII mode
int smi_miimode_check_state(smi_interface_t &smi) {
    return (smi_reg(smi, SPECIAL_MODES_REG, 0, SMI_READ) >> MIIMODE_BIT) & 1;
}
