...
// overrides for module_avb_1722_1/src/avb_1722_1_app_hooks.h

#if AVB_ENABLE_1722_1
    extern avb_1722_1_entity_record entities[AVB_1722_1_MAX_ENTITIES];
    static int controller_state = 0;

    // vendor ID correct? specific to Eval Board?
    // computed from the the MAC address of the primary network port on your device
    // ifndef define in avb_1722_1/default_conf.h:41
    // 24-bit OUI (Organizationally Unique Identifier)
    //#define XMOS_VENDOR_ID 0x00229700
    #define BCAT_VENDOR_ID 0x00099d00
    //#define AVB_1722_1_ADP_VENDOR_ID 0x00229700u
    //#define AVB_1722_1_ADP_MODEL_ID 0x1234
    // also in avb_conf.h


    void bcat_avb_controller(int *change_stream, int *toggle_remote, chanend c_tx) {
    ...
    void avb_entity_on_new_entity_available(client interface avb_interface avb,
                const_guid_ref_t my_guid,
                avb_1722_1_entity_record *entity,
                chanend c_tx) {
        // If Talker is enabled, connect to the first XMOS listener we see
        if (AVB_BCAT_ENABLE_TALKER && AVB_1722_1_CONTROLLER_ENABLED) {
            if ((entity->vendor_id == BCAT_VENDOR_ID)
                    && ((entity->listener_capabilities & AVB_1722_1_ADP_LISTENER_CAPABILITIES_AUDIO_SINK)
    ...
    void avb_talker_on_listener_connect(client interface avb_interface avb,
                int source_num,
                const_guid_ref_t listener_guid) {
        avb_talker_on_listener_connect_default(avb, source_num, listener_guid);
    }

    void avb_talker_on_listener_connect_failed(client interface avb_interface avb,
                const_guid_ref_t my_guid,
                int source_num,
                const_guid_ref_t listener_guid,
                avb_1722_1_acmp_status_t status,
                chanend c_tx) {
    ...
    avb_1722_1_acmp_status_t avb_listener_on_talker_connect(client interface avb_interface avb,
            ...
            if (AVB_1722_1_CONTROLLER_ENABLED) {
                if ((talker_guid.l >> 40) != (BCAT_VENDOR_ID>>8)) {
                    ...
                    if (i != AVB_1722_1_MAX_ENTITIES) {
                        do_connect = 1;
                        if (AVB_BCAT_ENABLE_TALKER && AVB_BCAT_ENABLE_LISTENER && (talker_guid.l < my_guid.l)) {
            ...
            if (do_connect) {
                debug_printf("CONNECTING Listener sink #%d -> Talker stream %x%x, DA: ",
                            sink_num, stream_id[0], stream_id[1]); print_mac_ln(dest_addr);
            ...
            debug_printf("CONNECTING Listener : entity not found : ");
            print_guid_ln(talker_guid);
